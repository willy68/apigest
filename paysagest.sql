...
-- -----------------------------------------------------
-- Table `paysagest`.`root`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `paysagest`.`root` ;

CREATE TABLE IF NOT EXISTS `paysagest`.`root` (
...
CREATE TABLE IF NOT EXISTS `paysagest`.`entreprise` (
  ...
  INDEX `fk_entreprise_root_idx` (`root_id` ASC),
  CONSTRAINT `fk_entreprise_root`
    FOREIGN KEY (`root_id`)
    REFERENCES `paysagest`.`root` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
